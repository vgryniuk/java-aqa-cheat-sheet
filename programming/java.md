...
* У абстрактних методів не буває тіла.
* Абстрактний клас може містити, а може і не містити абстрактні методи.
* Якщо в класі є абстрактний метод, то цей клас повинен бути абстрактним.
* Дочірний клас повинен оверрайдити всі абстрактні методи батьківського класу, або теж бути абстрактним.
* Абстрактний клас не може бути final.
* Змінні не можуть бути abstract.
* У абстрактних класів є конструктор.
* Будь-який оверрайднутий метод може бути як abstract, і не abstract.
* Для методів неприпустиме поєднання: final abstract, private abstract, static abstract
...
* Неможливо створити об'єкт інтерфейсу, оскільки це не клас.
* Інтерфейс не має конструкторів.
* Access modifier у всіх топ-левел інтерфейсів або public, або default.
* Якщо не вказати самостійно, то компілятор додасть до визначення інтерфейсу слово abstract.
* Інтерфейс не може бути final.
* Методи інтерфейсу не можуть бути final.
* Якщо не вказати самостійно, то компілятор додасть до визначення всіх non-default (не access modifier) та non-static методів слова abstract і public.
* Зі змінних в інтерфейсі можуть бути лише константи, які повинні бути в ньому ініціалізовані.
* Якщо не вказати самостійно, то компілятор додасть до визначення всіх змінних слова public, final і static.
* Якщо клас, який імплементував інтерфейс не перевизначив його методи, то цей клас повинен бути абстрактним.
* нтерфейс може мати внутрішні класи, які за замовчувнням будуть статичні.
